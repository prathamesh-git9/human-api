...
import { deriveKEK, verifyPassphrase, generateDEK, generateSalt, wrapDEK, unwrapDEK, seal, open, sealString, openString, } from './index';
describe('Crypto utilities', () => {
    ...
    describe('KDF functions', () => {
        ...
        it('verifies correct passphrase', async () => {
            ...
            const keyMaterial = await crypto.subtle.importKey('raw', new TextEncoder().encode(testPassphrase), 'PBKDF2', false, ['deriveBits']);
            ...
            const isValid = await verifyPassphrase(testPassphrase, fixedSalt, kekKeyArray);
            expect(isValid).toBe(true);
        });
        it('rejects wrong passphrase', async () => {
            const result = await deriveKEK(testPassphrase);
            const isValid = await verifyPassphrase('wrong-passphrase', result.kdfSalt, result.kekKey);
    ...
    describe('AES-GCM functions', () => {
        ...
        describe('DEK wrapping', () => {
            ...
            it('fails to unwrap with wrong KEK', async () => {
                ...
                const wrapped = await wrapDEK(kekKey, dek);
                await expect(unwrapDEK(wrongKEK, wrapped.wrappedDEK, wrapped.nonce)).rejects.toThrow('DEK unwrapping failed');
            });
            it('fails to unwrap with tampered data', async () => {
                const wrapped = await wrapDEK(kekKey, dek);
                const tampered = new Uint8Array(wrapped.wrappedDEK);
                tampered[0] ^= 1; // Flip one bit
                await expect(unwrapDEK(kekKey, tampered, wrapped.nonce)).rejects.toThrow('DEK unwrapping failed');
            });
        ...
        describe('Data encryption', () => {
            ...
            it('fails to decrypt with wrong DEK', async () => {
                ...
                const encrypted = await seal(dek, testData);
                await expect(open(wrongDEK, encrypted.cipher, encrypted.nonce)).rejects.toThrow('Decryption failed');
            });
            it('fails to decrypt with tampered cipher', async () => {
                const encrypted = await seal(dek, testData);
                const tampered = new Uint8Array(encrypted.cipher);
                tampered[0] ^= 1; // Flip one bit
                await expect(open(dek, tampered, encrypted.nonce)).rejects.toThrow('Decryption failed');
            });
            it('fails to decrypt with wrong nonce', async () => {
                const encrypted = await seal(dek, testData);
                const wrongNonce = crypto.getRandomValues(new Uint8Array(12));
                await expect(open(dek, encrypted.cipher, wrongNonce)).rejects.toThrow('Decryption failed');
            });
        });
