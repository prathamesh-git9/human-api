 ...
 */
export async function deriveKEK(passphrase, config = {}) {
    ...
    try {
        ...
        const kekKey = await crypto.subtle.deriveBits({
            ...
            hash: 'SHA-256',
        }, keyMaterial, kdfConfig.hashLength * 8 // Convert bytes to bits
        );
        return {
            kdfSalt,
 ...
 */
export async function verifyPassphrase(passphrase, kdfSalt, expectedKey) {
    try {
        // Import passphrase as key material
        const keyMaterial = await crypto.subtle.importKey('raw', new TextEncoder().encode(passphrase), 'PBKDF2', false, ['deriveBits']);
