  ...
  describe('AES-GCM functions', () => {
    ...
    describe('DEK wrapping', () => {
      ...
      it('fails to unwrap with wrong KEK', async () => {
        ...
        await expect(unwrapDEK(wrongKEK, wrapped.wrappedDEK, wrapped.nonce)).rejects.toThrow(
      ...
      it('fails to unwrap with tampered data', async () => {
        ...
        await expect(unwrapDEK(kekKey, tampered, wrapped.nonce)).rejects.toThrow(
          'DEK unwrapping failed'
        );
      });
      ...
      it('throws error for invalid DEK size', async () => {
        ...
        await expect(wrapDEK(kekKey, invalidDEK)).rejects.toThrow(
          'DEK must be 256 bits (32 bytes)'
        );
      });
    });
    ...
    describe('Data encryption', () => {
      ...
      it('fails to decrypt with wrong DEK', async () => {
        ...
        await expect(open(wrongDEK, encrypted.cipher, encrypted.nonce)).rejects.toThrow(
          'Decryption failed'
        );
      });
