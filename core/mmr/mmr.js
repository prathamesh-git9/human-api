 ...
 */
function calculateRelevance(queryVec, itemVec) {
    return cosineSimilarity(queryVec, itemVec);
}
/**
 * Calculate diversity score (max similarity to already selected items)
 */
function calculateDiversity(candidateId, selectedItems, allItems) {
 ...
 */
export function mmr(items, queryVec, lambda = 0.5, k = 12) {
    if (items.length === 0)
        return [];
    if (k <= 0)
        return [];
    ...
    if (k >= items.length) {
        return items.sort((a, b) => b.baseScore - a.baseScore).map(item => item.id);
    }
    const selected = [];
 ...
 */
export function mmrWithConfig(items, queryVec, config) {
